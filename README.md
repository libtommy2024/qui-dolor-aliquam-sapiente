# @libtommy2024/qui-dolor-aliquam-sapiente
...
1. Install @libtommy2024/qui-dolor-aliquam-sapiente:

   ```
   npm install --save-dev @libtommy2024/qui-dolor-aliquam-sapiente
   ```

2. Add @libtommy2024/qui-dolor-aliquam-sapiente to your ESLint configuration – either to [eslintrc] or to [eslint.config.js (flat config)].
   ...
   - eslint.config.js (flat config): Import @libtommy2024/qui-dolor-aliquam-sapiente, and put it in the configuration array – **after** other configs that you want to override.

     <!-- prettier-ignore -->
     ```js
     import someConfig from "some-other-config-you-use";
     import eslintConfigPrettier from "@libtommy2024/qui-dolor-aliquam-sapiente";
...
@libtommy2024/qui-dolor-aliquam-sapiente not only turns off _core_ rules, but also some from these plugins automatically:
...
> ℹ️ Note: You might find guides on the Internet saying you should also extend stuff like `"prettier/react"`. Since version 8.0.0 of @libtommy2024/qui-dolor-aliquam-sapiente, all you need to extend is `"prettier"`! That includes all plugins.
...
You might expect @libtommy2024/qui-dolor-aliquam-sapiente to turn off `ts/indent`, but it won’t! Because @libtommy2024/qui-dolor-aliquam-sapiente only turns off `@typescript-eslint/indent`. It cannot know what you chose to call the plugin. Same thing for the CLI helper tool.
...
Some of the rules that @libtommy2024/qui-dolor-aliquam-sapiente turns off may be deprecated, or even removed from ESLint. **This is perfectly fine,** but if you really need to omit the deprecated and removed rules, you can do so by setting the `ESLINT_CONFIG_PRETTIER_NO_DEPRECATED` environment variable to a non-empty value. For example:
...
@libtommy2024/qui-dolor-aliquam-sapiente also ships with a little CLI tool to help you check if your configuration contains any rules that are unnecessary or conflict with Prettier. Here’s how to run it:

```
npx @libtommy2024/qui-dolor-aliquam-sapiente path/to/main.js
```
...
With the new ESLint “flat config” format, you can control what things override what yourself. One way of solving the above conflict is to reorder the config objects so that @libtommy2024/qui-dolor-aliquam-sapiente is last:
...
export default [
  ...
  eslintConfigPrettier, // @libtommy2024/qui-dolor-aliquam-sapiente last
];
```

However, looking at the above config might feel confusing. It looks like we enable the `indent` rule, but in reality it’s disabled thanks to the `eslintConfigPrettier` line below it. Instead you might want to actually have your own rules _after_ @libtommy2024/qui-dolor-aliquam-sapiente and run the CLI helper tool to find out about problems, so you can remove conflicting rules from the config file altogether (simplifying your config).
...
```
npx @libtommy2024/qui-dolor-aliquam-sapiente index.js test/index.js legacy/main.js
```
...
Just like ESLint itself, you can control the @libtommy2024/qui-dolor-aliquam-sapiente CLI helper tool using the `ESLINT_USE_FLAT_CONFIG` environment variable:
...
@libtommy2024/qui-dolor-aliquam-sapiente versions before 7.0.0 had a slightly different CLI tool that was run in a different way. For example:

```
npx eslint --print-config index.js | npx @libtommy2024/qui-dolor-aliquam-sapiente-check
```

If you find something like that in a tutorial, this is what the command looks like in 7.0.0 or later:

```
npx @libtommy2024/qui-dolor-aliquam-sapiente index.js
```

## Special rules

There a few rules that @libtommy2024/qui-dolor-aliquam-sapiente disables that actually can be enabled in some cases.
...
See [package.json] for the exact versions of ESLint, Prettier and ESLint plugins that @libtommy2024/qui-dolor-aliquam-sapiente has been tested with.
...
`test-lint/foobar.js` must fail when used with eslint-plugin-foobar and eslint-plugin-prettier at the same time – until @libtommy2024/qui-dolor-aliquam-sapiente is added to the ESLint config. The file should be formatted according to Prettier, and that formatting should disagree with the plugin.
...
- `"test:eslint"` makes sure that the files in `test-lint/` pass ESLint when the exclusions from @libtommy2024/qui-dolor-aliquam-sapiente are used. It also lints the code of @libtommy2024/qui-dolor-aliquam-sapiente itself.
...
[package.json]: https://github.com/libtommy2024/qui-dolor-aliquam-sapiente/blob/main/package.json
